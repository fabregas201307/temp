...
# We modified the TF MeanIoU metric such that it works with one-hot encoded output
# The only place we modified is the first two lines of method `update_state`
...
class MeanIoU(Metric):
    ...
    def update_state(self, y_true, y_pred, sample_weight=None):
        ...
        y_true = math_ops.cast(math_ops.argmax(y_true, axis=-1), self._dtype)
        y_pred = math_ops.cast(math_ops.argmax(y_pred, axis=-1), self._dtype)
